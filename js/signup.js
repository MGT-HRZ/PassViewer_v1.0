...
const confirmPasswordEmptyError = document.getElementById('confirm-password-empty-error');
const confirmPasswordMismatchError = document.getElementById('confirm-password-mismatch-error');
...
/* This function is use to see whether both passwords are the same or not. */
...
function checkSubmitButton() {
    ...
    const doPasswordsMatch = passwordInput.value === confirmPasswordInput.value;  // Check if passwords match

    // Check if the verification code is valid
    const isVerificationCodeValid = validateVerificationCode();  // Validate 6-digit code

    // Check if terms are checked
    const checkedTermsValid = checkedTerms.checked === true;

    // Enable/Disable submit button based on the above conditions
    if (isPasswordValid && doPasswordsMatch && isVerificationCodeValid && checkedTermsValid) {
        submitEnable(submitButtonEnable);  // Enable submit button if all conditions are met
        
        submitButtonEnable.addEventListener('click', () => {
            setTimeout(() => {
                submitUnable(submitButtonEnable);
            }, 150);
        });
...
confirmPasswordInput.addEventListener('input', checkSubmitButton);
...
checkedTerms.addEventListener('change', checkSubmitButton);
...
resetButtonField.addEventListener('click', () => {
    ...
    toggleConfirmPasswordVisibility.querySelector('i').classList.add('fa-eye-slash');
    toggleConfirmPasswordVisibility.querySelector('i').classList.remove('fa-eye');
    ...
    confirmPasswordInput.classList.remove('border-red-500', 'dark:border-red-500', 'border-green-500', 'dark:border-green-500');
