...
<aside
            ...
            <li>
                <?php
                    try {
                        ...
                        $db_sidebar = new PDO('sqlite:../../database/main/PassViewer.db');
                        $db_sidebar->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);

                        // Prepare the SQL query
                        $stmt = $db_sidebar->prepare("SELECT id, sec_verification_code FROM users WHERE email = :email AND id = :target_id");
                        $stmt->bindParam(':email', $_SESSION['user_email'], PDO::PARAM_STR);
                        $stmt->bindParam(':target_id', $_SESSION['user_id'], PDO::PARAM_INT);
                        $stmt->execute();

                        // Fetch the user data
                        $user_unlock = $stmt->fetch(PDO::FETCH_ASSOC);

                        if ($user_unlock) {
                            // echo '<script>alert("Db connected");</script>';
                            if ($user_unlock['sec_verification_code'] === '-') {
                ?>
                        <form action="create-pin.php" method="POST" class="space-y-4 p-2">
                            <label for="password" class="text-gray-900 dark:text-white text-base font-medium">Unlock PIN</label>
                            <input
                                type="number"
                                id="pin"
                                name="pin"
                                placeholder="Create unlock PIN"
                                class="w-full p-2 text-sm bg-gray-50 border border-gray-300 rounded-full focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                maxlength="6"
                                required
                            />
                            <button
                                type="submit"
                                class="w-full bg-primary-500 text-white p-2 rounded-full hover:bg-primary-600 focus:outline-none focus:ring-2 focus:ring-primary-500 dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-500">
                                Create
                            </button>
                        </form>
                <?php
                
                            } elseif (isset($_GET['unlock']) && $_GET['unlock'] === $unlock_id) {
                            
                                echo '<form action="lock-view.php" method="POST">
                                    <button type="submit" id="lock" class="w-full bg-yellow-500 text-white font-semibold p-2 rounded-full hover:bg-yellow-600 focus:outline-none focus:ring-2 focus:ring-yellow-500 dark:bg-yellow-600 dark:hover:bg-yellow-700 dark:focus:ring-yellow-500">
                                        Lock <i class="fa-solid fa-lock fa-xs"></i>
                                    </button>
                                </form>';
                ...
                ?>
                        ...
                        <form action="unlock-pin.php" method="POST" class="space-y-4 p-2">
                            <label for="password" class="text-gray-900 dark:text-white text-base font-medium">Unlock PIN</label>
                            <input
                                type="number"
                                id="pin"
                            ...
                            <button
                                type="submit"
                                class="w-full bg-primary-500 text-white p-2 rounded-full hover:bg-primary-600 focus:outline-none focus:ring-2 focus:ring-primary-500 dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-500">
                                Unlock <i class="fa-solid fa-lock-open fa-xs"></i>
                            </button>
                        </form>
    ...
    <div
        ...
        <!-- <button data-tooltip-target="tooltip-theme" id="theme-toggle" class="inline-flex justify-center p-2 text-gray-500 rounded cursor-pointer dark:hover:text-white dark:text-gray-400 hover:text-gray-900 hover:bg-gray-100 dark:hover:bg-gray-600">
            <svg id="light-icon" xmlns="http://www.w3.org/2000/svg" class="hidden h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                <circle cx="12" cy="12" r="5" stroke="currentColor" stroke-width="2" fill="none" />
                <path stroke-linecap="round" stroke-linejoin="round" d="M12 1v3M12 20v3M4.222 4.222l2.121 2.121M17.657 17.657l2.121 2.121M1 12h3M20 12h3M4.222 19.778l2.121-2.121M17.657 6.343l2.121-2.121" />
            </svg>

            <svg id="dark-icon" xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
